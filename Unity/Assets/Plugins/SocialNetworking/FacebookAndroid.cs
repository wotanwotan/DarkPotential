...
public class FacebookAndroid
{
	...
	// Logs in a user attempting to use SSO if the Facebook app is available
	public static void loginWithSingleSignOn( string[] permissions )
	{
		if( Application.platform != RuntimePlatform.Android )
			return;

		var loginMethod = AndroidJNI.GetMethodID( _facebookPlugin.GetRawClass(), "showSSOLoginDialog", "([Ljava/lang/String;)V" );
		AndroidJNI.CallVoidMethod( _facebookPlugin.GetRawObject(), loginMethod, AndroidJNIHelper.CreateJNIArgArray( new object[] { permissions } ) );
	}
